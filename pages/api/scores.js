    ...
    const games = events.map(event => {
      ...
      return {
        ...
        odds: (() => {
          const o = comp.odds?.[0];
          if (!o) return null;
          // ESPN NCAAMB structure: moneyline, pointSpread, total are top-level objects
          // Each has .home.close/.away.close or .home.odds/.away.odds etc.
          const ml = o.moneyline || {};
          const ps = o.pointSpread || {};
          const tot = o.total || {};

          // Try multiple known paths for moneyline values
          const homeML = ml?.home?.close ?? ml?.home?.odds ?? ml?.home?.value ?? ml?.homeOdds ?? null;
          const awayML = ml?.away?.close ?? ml?.away?.odds ?? ml?.away?.value ?? ml?.awayOdds ?? null;

          // Spread values from pointSpread object or top-level
          const homeSpread = ps?.home?.close ?? ps?.home?.value ?? ps?.homeSpread ?? null;
          const awaySpread = ps?.away?.close ?? ps?.away?.value ?? ps?.awaySpread ?? null;
          ...
          return {
            ...
            // Debug: dump top-level moneyline structure
            _raw: {
              mlKeys: Object.keys(ml),
              mlHome: ml?.home ? (typeof ml.home === "object" ? Object.keys(ml.home) : ml.home) : null,
              mlAway: ml?.away ? (typeof ml.away === "object" ? Object.keys(ml.away) : ml.away) : null,
              mlRaw: JSON.stringify(ml).slice(0, 300),
              psRaw: JSON.stringify(ps).slice(0, 300),
            },
          };
        })(),
