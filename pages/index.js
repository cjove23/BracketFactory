...
// Background injury map — managed in code, not by users
...
// Injury display lookup
const INJURY_DISPLAY = {};
for (const inj of DEFAULT_INJURIES) {
  if (!inj.active) continue;
  if (!INJURY_DISPLAY[inj.team]) INJURY_DISPLAY[inj.team] = [];
  INJURY_DISPLAY[inj.team].push(inj);
}
...
export default function Home() {
  ...
  const bettingData=useMemo(()=>{
    const espnByTeam={};
    for(const g of live.liveGames){if(g.status==="final")continue;espnByTeam[g.home.name]=g;espnByTeam[g.away.name]=g;}
    return currentMatchups.map(m=>{
      ...
      let spreadEdge=marketSpread!=null?marketSpread-mSpread:null;
      ...
      // Even when spread is close/exact, ML can still have edge — always check both
      const maxEdge=Math.max(Math.abs(hiMLEdge||0),Math.abs(loMLEdge||0),Math.abs(spreadEdge||0));
      let rating="NO LINE";
      if(odds){if(maxEdge>=10)rating="STRONG VALUE";else if(maxEdge>=5)rating="VALUE";else if(maxEdge>=2)rating="LEAN";else rating="FAIR";}
      // Determine value team from best available edge
      let valueTeam=null,valueSide=null,bestEdge=0,bestKelly=null;
      const edges=[
        {side:"hi",team:m.hiName,edge:hiMLEdge,kelly:hiKelly,type:"ML"},
        {side:"lo",team:m.loName,edge:loMLEdge,kelly:loKelly,type:"ML"},
        {side:spreadEdge!=null&&spreadEdge>0?"hi":"lo",team:spreadEdge!=null&&spreadEdge>0?m.hiName:m.loName,edge:spreadEdge!=null?Math.abs(spreadEdge):null,kelly:null,type:"spread"},
      ];
      for(const e of edges){if(e.edge!=null&&e.edge>bestEdge){bestEdge=e.edge;valueTeam=e.team;valueSide=e.side;bestKelly=e.kelly;}}
      return{key:`${m.region}-${m.round}-${m.hiSeed}-${m.loSeed}`,round:m.round,region:m.region,hiSeed:m.hiSeed,loSeed:m.loSeed,hiName:m.hiName,loName:m.loName,modelSpread:mSpread,hiWinP,loWinP,marketSpread,hiML,loML,overUnder,spreadEdge,hiMLEdge,loMLEdge,hiKelly,loKelly,rating,hasOdds:!!odds,valueTeam,bestEdge,bestKelly};
    }).filter(Boolean);
  },[currentMatchups,live.liveGames]);
        ...
        <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
          ...
          <div style={{textAlign:"center",marginBottom:20}}>
            ...
            <p style={{color:C.textMuted,fontSize:12,margin:"6px 0 0"}}>
              KenPom-calibrated · Live scores via ESPN · {lockedCount} locked
          ...
          {tab==="betting"&&(
            <div style={{background:C.surface,border:`1px solid rgba(234,179,8,0.25)`,borderRadius:12,padding:18}}>
              ...
              <p style={{fontSize:10,color:C.textMuted,margin:"4px 0 10px"}}>
                Model spread & win % from KenPom + injury adjustments. ESPN odds overlay when available.
                {live.lastUpdate&&<span style={{color:C.textDim,marginLeft:6}}>Updated: {new Date(live.lastUpdate).toLocaleTimeString()}</span>}
              </p>

              {/* Filter buttons */}
              <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
                {["ALL","STRONG VALUE","VALUE","LEAN","FAIR","NO LINE"].map(f=>{
                  const ct=f==="ALL"?bettingData.length:bettingData.filter(g=>g.rating===f).length;
                  const col=f==="STRONG VALUE"?C.green:f==="VALUE"?C.blue:f==="LEAN"?C.accent:f==="FAIR"?C.textDim:C.textMuted;
              ...
              {(()=>{
                const RO={"STRONG VALUE":0,"VALUE":1,"LEAN":2,"FAIR":3,"NO LINE":4};
                const filtered=bettingFilter==="ALL"?bettingData:bettingData.filter(g=>g.rating===bettingFilter);
                const sorted=[...filtered].sort((a,b)=>(RO[a.rating]??5)-(RO[b.rating]??5));
                return sorted.length>0?(
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    {sorted.map(g=>{
                      const rc=g.rating==="STRONG VALUE"?C.green:g.rating==="VALUE"?C.blue:g.rating==="LEAN"?C.accent:C.textMuted;
                      return(
                        <div key={g.key} style={{background:C.card,border:`1px solid ${g.rating==="STRONG VALUE"||g.rating==="VALUE"?"rgba(234,179,8,0.3)":C.border}`,borderRadius:10,padding:14}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                            ...
                            <span style={{background:`${rc}22`,border:`1px solid ${rc}44`,color:rc,padding:"3px 10px",borderRadius:4,fontSize:9,fontWeight:800}}>
                              {g.rating}{g.valueTeam&&g.rating!=="FAIR"&&g.rating!=="NO LINE"?` — ${g.valueTeam}`:""}
                            </span>
                          </div>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,fontSize:10,fontFamily:"monospace"}}>
                            ...
                            <div style={{background:C.surface,borderRadius:6,padding:"8px 10px"}}>
                              ...
                              {g.marketSpread!=null?(<><div style={{color:C.text,fontWeight:700,fontSize:14}}>{g.marketSpread>0?"+":""}{g.marketSpread.toFixed(1)}</div>{g.spreadEdge!=null&&<div style={{fontSize:8,color:Math.abs(g.spreadEdge)>=3?C.yellow:C.textMuted}}>Edge: {g.spreadEdge>0?"+":""}{g.spreadEdge.toFixed(1)} pts</div>}</>):(<div style={{color:C.textMuted,fontSize:11}}>No line yet</div>)}
                            </div>
                            <div style={{background:C.surface,borderRadius:6,padding:"8px 10px"}}>
                          ...
                          {g.hasOdds&&g.bestEdge>2&&(<div style={{marginTop:10,padding:"8px 12px",background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:6}}>
                            <span style={{fontSize:10,color:C.yellow,fontWeight:700}}>💰 Best value: {g.valueTeam}</span>
                            <span style={{fontSize:10,color:C.textDim,marginLeft:8}}>Edge +{g.bestEdge.toFixed(1)}%{g.bestKelly>0&&` · Kelly: ${(g.bestKelly*100).toFixed(1)}%`}</span>
                          </div>)}
                        </div>
                      );
                    })}
              ...
              <div style={{marginTop:16,padding:12,background:"rgba(234,179,8,0.06)",borderRadius:8,border:"1px solid rgba(234,179,8,0.15)"}}>
                ...
                <div style={{fontSize:9,color:C.textMuted,lineHeight:1.6}}>
                  Statistical model, not financial advice. Edge = model probability minus market implied probability. Kelly criterion shows theoretical optimal sizing — most use ¼ to ½ Kelly. Does not account for public action or game-day factors.
                </div>
              </div>
            </div>
          )}
