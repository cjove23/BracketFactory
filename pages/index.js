...
export default function Home() {
  ...
  // Combine picks: server (Redis, permanent) preferred, client-side as fallback
  const combinedPicks = useMemo(() => {
    const serverGraded = (live.serverPicks || []).filter(p => p.result);
    if (serverGraded.length > 0) return serverGraded;
    return picksRecord; // Fallback to client-side when Redis has nothing
  }, [live.serverPicks, picksRecord]);

  const recordW=combinedPicks.filter(p=>p.result==="W").length;
  const recordL=combinedPicks.filter(p=>p.result==="L").length;
  const recordP=combinedPicks.filter(p=>p.result==="P").length;

  // P/L calculation — $100 flat bets, spreads at -110, ML at listed odds
  const recordPL=combinedPicks.reduce((total,p)=>{
    if(p.result==="P")return total;
    if(p.result==="L")return total-100;
    const type=p.usedType||p.bestType;
    if(type==="ML"){
      const ml=p.usedLine!=null?p.usedLine:p.bestLine;
      ...
      <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Outfit',sans-serif"}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
          ...
          <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
            {[{key:"live",label:"SCOREBOARD"},{key:"bracket",label:"BRACKET"},{key:"upsets",label:"UPSET MAKERS"},{key:"betting",label:"BETTING"},{key:"record",label:"RECORD"+(combinedPicks.length>0?` (${recordW}-${recordL})`:"")},{key:"champion",label:"CHAMPIONS"}].map(t=>
          ...
          {/* RECORD TAB */}
          {tab==="record"&&(()=>{
            const filteredPicks=recordFilter==="ALL"?combinedPicks:combinedPicks.filter(p=>p.rating===recordFilter);
            ...
            <div style={{background:C.surface,border:`1px solid rgba(34,197,94,0.25)`,borderRadius:12,padding:18}}>
              ...
              <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
                {["ALL","SHARP","+EV","STRONG VALUE","VALUE"].map(f=>{
                  const ct=f==="ALL"?combinedPicks.length:combinedPicks.filter(p=>p.rating===f).length;
                  if(f!=="ALL"&&ct===0)return null;
                  const w=f==="ALL"?recordW:combinedPicks.filter(p=>p.rating===f&&p.result==="W").length;
                  const l=f==="ALL"?recordL:combinedPicks.filter(p=>p.rating===f&&p.result==="L").length;
                ...
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"monospace",fontSize:10}}>
                    ...
                    <tbody>{filteredPicks.map(p=>{
                      ...
                      return(
                        <tr key={p.gameId||p.hiName+"|"+p.loName} style={{borderBottom:`1px solid rgba(42,53,80,0.3)`}}>
                          ...
                          <td style={{padding:"5px 8px",color:C.textDim}}>{(p.usedType||p.bestType)==="ML"?"ML":"Spread"}</td>
                          <td style={{padding:"5px 8px",color:C.text,fontWeight:600}}>
                            {(()=>{const t=p.usedType||p.bestType;const l=p.usedLine!=null?p.usedLine:p.bestLine;if(t==="ML")return(l>0?"+":"")+l;const ts=p.valueTeam===p.hiName?(p.marketSpread||0):-(p.marketSpread||0);return(ts>0?"+":"")+ts.toFixed(1);})()}
                          </td>
                          <td style={{padding:"5px 8px",color:C.yellow,fontWeight:700}}>+{(typeof p.bestEdge==="number"?p.bestEdge:0).toFixed(1)}%</td>
