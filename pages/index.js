...
export default function Home() {
  ...
  // ── Betting Analysis ──
  // Uses currentMatchups (bracket-derived) as base, overlays ESPN odds when available
  const bettingData = useMemo(() => {
    // Build ESPN game lookup by team names
    const espnByTeam = {};
    ...
    return currentMatchups.map(m => {
      const hiInfo = TEAM_LOOKUP[m.hiName];
      const loInfo = TEAM_LOOKUP[m.loName];
      ...
      // Model predictions (hi seed = "home" equivalent, lo seed = "away")
      ...
      // Find matching ESPN game for odds
      const espnGame = espnByTeam[m.hiName] || espnByTeam[m.loName];
      const odds = espnGame?.odds;
      let marketSpread = null, hiML = null, loML = null, overUnder = null;
      let espnStatus = null, espnTime = null;

      if (espnGame) {
        espnStatus = espnGame.status;
        espnTime = espnGame.startTime;
        if (odds) {
          marketSpread = odds.spread != null ? parseFloat(odds.spread) : null;
          overUnder = odds.overUnder != null ? parseFloat(odds.overUnder) : null;
          // Figure out which ESPN side is our hi seed vs lo seed
          const hiIsHome = espnGame.home.name === m.hiName;
          hiML = hiIsHome ? (odds.homeML != null ? parseInt(odds.homeML) : null) : (odds.awayML != null ? parseInt(odds.awayML) : null);
          loML = hiIsHome ? (odds.awayML != null ? parseInt(odds.awayML) : null) : (odds.homeML != null ? parseInt(odds.homeML) : null);
          // Adjust spread sign: ESPN spread is relative to home team, normalize to hi seed perspective
      ...
      // Spread edge
      let spreadEdge = null;
      if (marketSpread != null) spreadEdge = marketSpread - mSpread;

      // ML edge
      let hiMLEdge = null, loMLEdge = null, hiKelly = null, loKelly = null;
      if (hiML) {
        const impliedP = oddsToProb(hiML);
        hiMLEdge = impliedP != null ? ((hiWinP - impliedP) * 100) : null;
        const dec = americanToDecimal(hiML);
        hiKelly = dec ? kellyCriterion(hiWinP, dec) : null;
      }
      if (loML) {
        const impliedP = oddsToProb(loML);
        loMLEdge = impliedP != null ? ((loWinP - impliedP) * 100) : null;
        const dec = americanToDecimal(loML);
        loKelly = dec ? kellyCriterion(loWinP, dec) : null;
      }

      const maxEdge = Math.max(Math.abs(hiMLEdge||0), Math.abs(loMLEdge||0), Math.abs(spreadEdge||0));
      let rating = "NO LINE";
      if (odds) {
      ...
      return {
        key: `${m.region}-${m.round}-${m.hiSeed}-${m.loSeed}`,
        round: m.round, region: m.region,
        hiSeed: m.hiSeed, loSeed: m.loSeed, hiName: m.hiName, loName: m.loName,
        modelSpread: mSpread, hiWinP, loWinP,
        marketSpread, hiML, loML, overUnder,
        spreadEdge, hiMLEdge, loMLEdge, hiKelly, loKelly,
        rating, hasOdds: !!odds,
        espnStatus, espnTime,
      };
    }).filter(Boolean);
  }, [currentMatchups, live.liveGames, injuryMap]);
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Outfit',sans-serif"}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
          ...
          {/* ── INJURY TRACKER (minimal) ── */}
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,marginBottom:16,fontSize:10,fontFamily:"monospace",flexWrap:"wrap"}}>
            <span style={{color:C.red,fontWeight:700,fontSize:9}}>🩹 {activeInjuries.length} INJURIES</span>
            <span style={{color:C.textDim}}>{activeInjuries.map(i=>`${i.team} (${i.emAdj})`).join(" · ")||"None"}</span>
            <button onClick={()=>setShowInjuryPanel(!showInjuryPanel)} style={{
              background:"transparent",border:`1px solid ${C.border}`,color:C.textMuted,borderRadius:4,padding:"2px 8px",cursor:"pointer",fontSize:9,fontFamily:"'Outfit',sans-serif",marginLeft:"auto",
            }}>{showInjuryPanel?"CLOSE":"EDIT"}</button>
            {showInjuryPanel&&(
              <div style={{width:"100%",paddingTop:10,borderTop:`1px solid ${C.border}`,marginTop:4}}>
                {injuries.map(inj=>(
                  <div key={inj.id} style={{display:"flex",alignItems:"center",gap:8,padding:"3px 0",opacity:inj.active?1:0.4}}>
                    <input type="checkbox" checked={inj.active} onChange={()=>toggleInjury(inj.id)} style={{accentColor:C.red,cursor:"pointer"}}/>
                    <span style={{color:C.accent,fontWeight:700,minWidth:90}}>{inj.team}</span>
                    <span style={{color:C.text}}>{inj.player}</span>
                    <span style={{color:C.textMuted}}>({inj.type})</span>
                    <span style={{color:C.red,fontWeight:700}}>{inj.emAdj}</span>
                    <button onClick={()=>removeInjury(inj.id)} style={{background:"transparent",border:"none",color:C.textMuted,cursor:"pointer",fontSize:11,padding:0}}>✕</button>
                  </div>
                ))}
                <div style={{display:"flex",gap:6,alignItems:"center",marginTop:8,paddingTop:8,borderTop:`1px solid rgba(42,53,80,0.3)`}}>
                  <select value={newInj.team} onChange={e=>setNewInj({...newInj,team:e.target.value})}
                    style={{background:C.card,border:`1px solid ${C.border}`,color:C.text,borderRadius:3,padding:"3px 4px",fontSize:9,maxWidth:120}}>
                    {ALL_TEAM_NAMES.map(n=><option key={n} value={n}>{n}</option>)}
                  </select>
                  <input placeholder="Player" value={newInj.player} onChange={e=>setNewInj({...newInj,player:e.target.value})}
                    style={{background:C.card,border:`1px solid ${C.border}`,color:C.text,borderRadius:3,padding:"3px 6px",fontSize:9,width:90}}/>
                  <input placeholder="Type" value={newInj.type} onChange={e=>setNewInj({...newInj,type:e.target.value})}
                    style={{background:C.card,border:`1px solid ${C.border}`,color:C.text,borderRadius:3,padding:"3px 6px",fontSize:9,width:70}}/>
                  <input type="number" step="0.5" min="-15" max="0" value={newInj.emAdj}
                    onChange={e=>setNewInj({...newInj,emAdj:parseFloat(e.target.value)||0})}
                    style={{background:C.card,border:`1px solid ${C.border}`,color:C.red,borderRadius:3,padding:"3px 4px",fontSize:9,width:45}}/>
                  <button onClick={addInjury} style={{background:C.red,border:"none",color:"#fff",borderRadius:3,padding:"3px 8px",cursor:"pointer",fontSize:9,fontWeight:700,opacity:newInj.player.trim()?1:0.4}}>+</button>
                </div>
              </div>
            )}
          </div>
            ...
            {tab==="betting"&&(
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={{background:C.surface,border:`1px solid rgba(234,179,8,0.25)`,borderRadius:12,padding:18}}>
                  ...
                  <p style={{fontSize:10,color:C.textMuted,margin:"0 0 14px"}}>
                    Compares our injury-adjusted KenPom model against market lines. Positive edge = model thinks the bet has value. Lines from ESPN when available — click REFRESH to update.
                  </p>
                  ...
                  {bettingData.length > 0 ? (
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {bettingData.map(g => {
                        ...
                        const bestEdge = Math.max(g.hiMLEdge||0, g.loMLEdge||0);
                        const bestSide = (g.hiMLEdge||0) > (g.loMLEdge||0) ? "hi" : "lo";
                        const bestTeam = bestSide==="hi" ? g.hiName : g.loName;
                        const bestKelly = bestSide==="hi" ? g.hiKelly : g.loKelly;
                        return (
                          <div key={g.key} style={{background:C.card,border:`1px solid ${g.rating==="STRONG VALUE"||g.rating==="VALUE"?"rgba(234,179,8,0.3)":C.border}`,borderRadius:10,padding:14}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                              <div style={{display:"flex",alignItems:"center",gap:8}}>
                                ...
                                <span style={{fontSize:11,fontWeight:700}}>
                                  <span style={{color:C.textDim}}>#{g.hiSeed} </span>
                                  <span style={{color:C.text}}>{g.hiName}</span>
                                  <span style={{color:C.textMuted,margin:"0 6px"}}>vs</span>
                                  <span style={{color:C.textDim}}>#{g.loSeed} </span>
                                  <span style={{color:C.text}}>{g.loName}</span>
                                </span>
                                <span style={{fontSize:9,color:C.textMuted}}>{g.region}</span>
                                {g.espnStatus==="live"&&<span style={{background:C.red,color:"#fff",fontSize:8,fontWeight:800,padding:"2px 6px",borderRadius:3}}>LIVE</span>}
                              </div>
                            ...
                            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,fontSize:10,fontFamily:"monospace"}}>
                              ...
                              <div style={{background:C.surface,borderRadius:6,padding:"8px 10px"}}>
                                ...
                                <div style={{display:"flex",justifyContent:"space-between"}}>
                                  <div><div style={{color:g.hiWinP>g.loWinP?C.green:C.textDim,fontWeight:700}}>{(g.hiWinP*100).toFixed(1)}%</div><div style={{fontSize:8,color:C.textMuted}}>#{g.hiSeed} {g.hiName}</div></div>
                                  <div style={{textAlign:"right"}}><div style={{color:g.loWinP>g.hiWinP?C.green:C.textDim,fontWeight:700}}>{(g.loWinP*100).toFixed(1)}%</div><div style={{fontSize:8,color:C.textMuted}}>#{g.loSeed} {g.loName}</div></div>
                                </div>
                              </div>
                              ...
                              <div style={{background:C.surface,borderRadius:6,padding:"8px 10px"}}>
                                ...
                                {(g.hiML||g.loML)?(
                                  <div style={{display:"flex",justifyContent:"space-between"}}>
                                    <div>
                                      <div style={{color:g.hiMLEdge&&g.hiMLEdge>2?C.yellow:C.text,fontWeight:700}}>{g.hiML>0?"+":""}{g.hiML||"—"}</div>
                                      {g.hiMLEdge!=null&&<div style={{fontSize:8,color:g.hiMLEdge>5?C.green:g.hiMLEdge>0?C.yellow:C.textMuted}}>Edge: {g.hiMLEdge>0?"+":""}{g.hiMLEdge.toFixed(1)}%</div>}
                                    </div>
                                    <div style={{textAlign:"right"}}>
                                      <div style={{color:g.loMLEdge&&g.loMLEdge>2?C.yellow:C.text,fontWeight:700}}>{g.loML>0?"+":""}{g.loML||"—"}</div>
                                      {g.loMLEdge!=null&&<div style={{fontSize:8,color:g.loMLEdge>5?C.green:g.loMLEdge>0?C.yellow:C.textMuted}}>Edge: {g.loMLEdge>0?"+":""}{g.loMLEdge.toFixed(1)}%</div>}
                                    </div>
                                  </div>
                                ):(<div style={{color:C.textMuted,fontSize:11}}>No line yet</div>)}
                              </div>
                    ...
                    <div style={{textAlign:"center",padding:"30px 20px",color:C.textMuted}}>
                      <div style={{fontSize:13}}>No upcoming matchups found. All games may be locked/completed.</div>
                    </div>
                  )}
