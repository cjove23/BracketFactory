  ...
  const picksRecord=useMemo(()=>{
    ...
    for(const r of REGION_NAMES){
      for(let s=1;s<=16;s++){
        ...
        const odds=espnGame.odds;
        if(!odds)continue;
        let marketSpread=odds.spread!=null?parseFloat(odds.spread):null;
        const hiIsHome=espnGame.home.name===hiName;
        let hiML=hiIsHome?(odds.homeML!=null?parseInt(odds.homeML):null):(odds.awayML!=null?parseInt(odds.awayML):null);
        let loML=hiIsHome?(odds.awayML!=null?parseInt(odds.awayML):null):(odds.homeML!=null?parseInt(odds.homeML):null);
        if(marketSpread!=null&&!hiIsHome)marketSpread=-marketSpread;
            ...
            <div style={{background:C.surface,border:`1px solid rgba(234,179,8,0.25)`,borderRadius:12,padding:18}}>
              ...
              <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
                ...
                {[{key:"ALL",label:"ALL"},{key:"spread",label:"SPREAD"},{key:"ML",label:"MONEYLINE"}].map(f=>{
                  const ct=f.key==="ALL"?bettingData.filter(g=>g.hasOdds||g.bestEdge>0).length:bettingData.filter(g=>g.bestType===f.key).length;
              ...
              {(()=>{
                let filtered=bettingFilter==="ALL"?bettingData:bettingData.filter(g=>g.rating===bettingFilter);
