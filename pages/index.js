  ...
  const bettingData=useMemo(()=>{
    ...
    return currentMatchups.map(m=>{
      ...
      const edges=[
        ...
        {side:spreadEdge!=null&&spreadEdge>0?"hi":"lo",team:spreadEdge!=null&&spreadEdge>0?m.hiName:m.loName,edge:spreadEdge!=null?Math.abs(spreadEdge):null,kelly:null,type:"spread",line:marketSpread},
      ];
                      ...
                      return(
                        <div key={g.key} style={{background:C.card,border:`1px solid ${g.rating==="STRONG VALUE"||g.rating==="VALUE"?"rgba(234,179,8,0.3)":C.border}`,borderRadius:10,padding:14}}>
                          ...
                          {g.hasOdds&&g.bestEdge>2&&(<div style={{marginTop:10,padding:"8px 12px",background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:6}}>
                            <span style={{fontSize:10,color:C.yellow,fontWeight:700}}>💰 {g.valueTeam} {g.bestType==="ML"?(g.bestLine>0?"+":"")+g.bestLine+" ML":(g.bestLine>0?"+":"")+g.bestLine.toFixed(1)+" spread"}</span>
