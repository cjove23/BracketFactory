...
export default function Home() {
  ...
  const bettingData=useMemo(()=>{
    ...
    return currentMatchups.map(m=>{
      ...
      return{key:`${m.region}-${m.round}-${m.hiSeed}-${m.loSeed}`,round:m.round,region:m.region,hiSeed:m.hiSeed,loSeed:m.loSeed,hiName:m.hiName,loName:m.loName,modelSpread:mSpread,hiWinP,loWinP,marketSpread,hiML,loML,overUnder,spreadEdge,hiMLEdge,loMLEdge,hiKelly,loKelly,rating,hasOdds:!!odds};
    }).filter(Boolean);
                      ...
                      return(
                        <div key={g.key} style={{background:C.card,border:`1px solid ${g.rating==="STRONG VALUE"||g.rating==="VALUE"?"rgba(234,179,8,0.3)":C.border}`,borderRadius:10,padding:14}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                            ...
                            <span style={{background:`${rc}22`,border:`1px solid ${rc}44`,color:rc,padding:"3px 10px",borderRadius:4,fontSize:9,fontWeight:800}}>{g.rating}</span>
                          </div>
                          ...
                          {g.hasOdds&&bestEdge>2&&(<div style={{marginTop:10,padding:"8px 12px",background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:6}}>
                            <span style={{fontSize:10,color:C.yellow,fontWeight:700}}>💰 Best value: {bestTeam}</span>
                            <span style={{fontSize:10,color:C.textDim,marginLeft:8}}>ML edge +{bestEdge.toFixed(1)}%{bestKelly>0&&` · Kelly: ${(bestKelly*100).toFixed(1)}%`}</span>
                          </div>)}
                        </div>
                      );
                    })}
