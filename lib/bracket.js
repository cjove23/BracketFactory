...
// Blowout amplifier: linear model underestimates how badly elite teams beat weak teams.
// When raw spread exceeds BLOWOUT_THRESHOLD, scale excess by BLOWOUT_SCALE.
// Keeps close games untouched, widens extreme spreads toward market reality.
export const BLOWOUT_THRESHOLD = 12; // spreads under 12 pts are unaffected
export const BLOWOUT_SCALE = 1.25;   // excess over threshold gets 25% boost

export function adjustedSpread(rawSpread) {
  const abs = Math.abs(rawSpread);
  if (abs <= BLOWOUT_THRESHOLD) return rawSpread;
  const sign = rawSpread > 0 ? 1 : -1;
  return sign * (BLOWOUT_THRESHOLD + (abs - BLOWOUT_THRESHOLD) * BLOWOUT_SCALE);
}

export function winProb(emA, tA, emB, tB, oA, dA, oB, dB) {
  const rawSpread = (emA - emB) * (tA + tB) / 200;
...
export function modelSpread(teamA, teamB, injuryMap) {
  ...
  const raw = -((emA - emB) * (teamA.adjT + teamB.adjT) / 200);
  return adjustedSpread(raw);
}
...
// Keyed by sorted team name pair. Spread from home team perspective.
// This ensures the Record tab is consistent across all devices.
export const KNOWN_ODDS = {
  // ── R64 Thursday ── (DraftKings)
  "Ohio St|TCU": { spread: -2.5, homeML: -142, awayML: 120, home: "Ohio St" },
  "Nebraska|Troy": { spread: -13.5, homeML: -1000, awayML: 650, home: "Nebraska" },
  ...
  // ── R64 Friday ── (DraftKings/FanDuel)
  "Kentucky|Santa Clara": { spread: -8.5, homeML: -420, awayML: 320, home: "Kentucky" },
  "Akron|Texas Tech": { spread: -14.5, homeML: -1200, awayML: 750, home: "Texas Tech" },
  "Arizona|LIU": { spread: -30.5, homeML: -20000, awayML: 3000, home: "Arizona" },
  "Virginia|Wright St": { spread: -13.5, homeML: -900, awayML: 600, home: "Virginia" },
  "Iowa St|Tennessee St": { spread: -25.5, homeML: -10000, awayML: 3000, home: "Iowa St" },
  "Alabama|Hofstra": { spread: -16.5, homeML: -1600, awayML: 900, home: "Alabama" },
  "Utah St|Villanova": { spread: -1.5, homeML: -120, awayML: 100, home: "Villanova" },
  "Miami (OH)|Tennessee": { spread: -13.5, homeML: -900, awayML: 600, home: "Tennessee" },
  "Clemson|Iowa": { spread: 2.5, homeML: 130, awayML: -155, home: "Clemson" },
  "N. Iowa|St. John's": { spread: -9.5, homeML: -480, awayML: 360, home: "St. John's" },
  "UCF|UCLA": { spread: -5.5, homeML: -230, awayML: 190, home: "UCLA" },
  "Purdue|Queens": { spread: -26.5, homeML: -12000, awayML: 3000, home: "Purdue" },
  "Florida|Prairie View": { spread: -30.5, homeML: -20000, awayML: 3000, home: "Florida" },
  "Cal Baptist|Kansas": { spread: -13.5, homeML: -900, awayML: 600, home: "Kansas" },
  "Furman|UConn": { spread: -20.5, homeML: -3500, awayML: 1400, home: "UConn" },
  "Miami (FL)|Missouri": { spread: -3.5, homeML: -170, awayML: 142, home: "Miami (FL)" },
  // ── R32 Saturday ── (DraftKings/BetMGM)
  "Duke|TCU": { spread: -10.5, homeML: -550, awayML: 400, home: "Duke" },
  "Michigan|Saint Louis": { spread: -12.5, homeML: -800, awayML: 550, home: "Michigan" },
  "Houston|Texas A&M": { spread: -8.5, homeML: -400, awayML: 310, home: "Houston" },
  "Nebraska|Vanderbilt": { spread: 1.5, homeML: 110, awayML: -130, home: "Nebraska" },
  "Illinois|VCU": { spread: -10.5, homeML: -550, awayML: 400, home: "Illinois" },
  "Arkansas|High Point": { spread: -11.5, homeML: -700, awayML: 500, home: "Arkansas" },
  "Louisville|Michigan St": { spread: 2.5, homeML: 125, awayML: -148, home: "Louisville" },
  "Gonzaga|Texas": { spread: -6.5, homeML: -280, awayML: 230, home: "Gonzaga" },
  // ── R32 Sunday ── (DraftKings/BetMGM/ESPN)
  "Miami (FL)|Purdue": { spread: -5.5, homeML: -230, awayML: 190, home: "Purdue" },
  "Iowa St|Kentucky": { spread: -4.5, homeML: -230, awayML: 190, home: "Iowa St" },
  "Kansas|St. John's": { spread: -3.5, homeML: -166, awayML: 140, home: "St. John's" },
  "Tennessee|Virginia": { spread: -1.5, homeML: -118, awayML: -102, home: "Tennessee" },
  "Florida|Iowa": { spread: -10.5, homeML: -600, awayML: 440, home: "Florida" },
  "Arizona|Utah St": { spread: -11.5, homeML: -800, awayML: 550, home: "Arizona" },
  "UCLA|UConn": { spread: -4.5, homeML: -198, awayML: 164, home: "UConn" },
  "Alabama|Texas Tech": { spread: 1.5, homeML: -102, awayML: -118, home: "Alabama" },
  // ── Sweet 16 ── (add as lines are published)
};

// ── Known Results — pre-seeded bracket locks ──
// Ensures auto-lock works even when ESPN old-date APIs don't return games.
// Format: locked[region][roundIdx][gameIdx] = winning seed
// SEED_ORDER = [1,16,8,9,5,12,4,13,6,11,3,14,7,10,2,15]
// R64 G0=1v16, G1=8v9, G2=5v12, G3=4v13, G4=6v11, G5=3v14, G6=7v10, G7=2v15
export const KNOWN_RESULTS = {
  East: {
    0: { 0:1, 1:9, 2:5, 3:4, 4:6, 5:3, 6:7, 7:2 },  // R64: Duke, TCU, St. John's, Kansas, Louisville, Michigan St, UCLA, UConn
    1: { 0:1, 1:5, 2:3, 3:2 },                         // R32: Duke, St. John's, Michigan St, UConn
  },
  South: {
    0: { 0:1, 1:9, 2:5, 3:4, 4:11, 5:3, 6:10, 7:2 }, // R64: Florida, Iowa, Vanderbilt, Nebraska, VCU, Illinois, Texas A&M, Houston
    1: { 0:9, 1:4, 2:3, 3:2 },                          // R32: Iowa(!), Nebraska, Illinois, Houston
  },
  West: {
    0: { 0:1, 1:9, 2:12, 3:4, 4:11, 5:3, 6:7, 7:2 }, // R64: Arizona, Utah St, High Point(!), Arkansas, Texas, Gonzaga, Miami FL, Purdue
    1: { 0:1, 1:4, 2:11, 3:2 },                         // R32: Arizona, Arkansas, Texas(!), Purdue
  },
  Midwest: {
    0: { 0:1, 1:9, 2:5, 3:4, 4:6, 5:3, 6:7, 7:2 },   // R64: Michigan, Saint Louis, Texas Tech, Alabama, Tennessee, Virginia, Kentucky, Iowa St
    1: { 0:1, 1:4, 2:6, 3:2 },                          // R32: Michigan, Alabama, Tennessee, Iowa St
  },
};
