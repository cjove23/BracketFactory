...
import { TEAMS, REGION_NAMES, SEED_ORDER, KNOWN_RESULTS } from "./bracket";
...
export default function useLiveScores() {
  ...
  const [locked, setLocked] = useState(() => {
    return JSON.parse(JSON.stringify(KNOWN_RESULTS || { East: {}, South: {}, West: {}, Midwest: {} }));
  });
  ...
  const [serverPicks, setServerPicks] = useState([]);
  const intervalRef = useRef(null);
  const lockedRef = useRef(locked);
  const lockedFFRef = useRef(lockedFF);
  const initialFetchDone = useRef(false);
  const serverLocksLoaded = useRef(false);
  const allGamesCache = useRef({});
  const knownInjuryKeys = useRef(new Set());
  lockedRef.current = locked;
  lockedFFRef.current = lockedFF;

  // Save locks to Redis (fire-and-forget)
  const saveLocks = useCallback((newLocked, newLockedFF) => {
    try {
      fetch("/api/locks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ locked: newLocked, lockedFF: newLockedFF }),
      }).catch(function(e) { /* best effort */ });
    } catch(e) { /* best effort */ }
  }, []);

  // Load locks from Redis on mount
  useEffect(() => {
    fetch("/api/locks").then(function(r) { return r.json(); }).then(function(data) {
      if (data.locked) {
        // Merge: Redis locks override KNOWN_RESULTS
        const merged = JSON.parse(JSON.stringify(KNOWN_RESULTS || { East: {}, South: {}, West: {}, Midwest: {} }));
        for (const region of REGION_NAMES) {
          if (data.locked[region]) {
            for (const rd in data.locked[region]) {
              if (!merged[region][rd]) merged[region][rd] = {};
              for (const gi in data.locked[region][rd]) {
                if (data.locked[region][rd][gi] != null) {
                  merged[region][rd][gi] = data.locked[region][rd][gi];
                }
              }
            }
          }
        ...
        setLocked(merged);
        lockedRef.current = merged;
      }
      if (data.lockedFF) {
        setLockedFF(data.lockedFF);
        lockedFFRef.current = data.lockedFF;
      }
      serverLocksLoaded.current = true;
    }).catch(function(e) { serverLocksLoaded.current = true; });
  }, []);

  // ── Persistent Odds Storage (localStorage) ──
  const oddsCache = useRef({});

  const saveOdds = useCallback((cache) => {
    try { if (typeof window !== "undefined") localStorage.setItem("bf_odds_cache", JSON.stringify(cache)); }
    catch(e) { /* storage full, private mode, or unavailable */ }
  }, []);

  // Load saved odds from localStorage on mount (client-side only)
  useEffect(() => {
    try {
      const saved = localStorage.getItem("bf_odds_cache");
      if (saved) {
        const parsed = JSON.parse(saved);
        oddsCache.current = parsed;
        setCachedOdds(parsed);
      }
    } catch(e) { /* unavailable */ }
  }, []);
  ...
  const fetchScores = useCallback(async () => {
    try {
      ...
      // Merge into persistent cache — update existing games, capture odds permanently
      let oddsChanged = false;
      for (const g of allGames) {
        // Capture odds to persistent localStorage cache while they exist
        if (g.odds && (g.odds.spread != null || g.odds.homeML != null)) {
          // Key by game ID and both team names for flexible lookup
          oddsCache.current[g.id] = g.odds;
          const teamKey = [g.home.name, g.away.name].sort().join("|");
          oddsCache.current[teamKey] = { ...g.odds, _home: g.home.name, _away: g.away.name };
          oddsChanged = true;
        }
        // Restore odds on final games from persistent cache
        if (g.status === "final" && (!g.odds || (g.odds.homeML == null && g.odds.spread == null))) {
          const teamKey = [g.home.name, g.away.name].sort().join("|");
          const cached = oddsCache.current[g.id] || oddsCache.current[teamKey];
          if (cached) g.odds = cached;
        }
        allGamesCache.current[g.id] = g;
      }
      if (oddsChanged) saveOdds(oddsCache.current);
      const mergedGames = Object.values(allGamesCache.current);

      setLiveGames(mergedGames);
      setCachedOdds({...oddsCache.current});
      ...
      // ── Fetch server-side picks (saved in Redis) ──
      try {
        const picksRes = await fetch("/api/picks");
        if (picksRes.ok) {
          const picksData = await picksRes.json();
          if (picksData.picks && picksData.picks.length > 0) {
            setServerPicks(picksData.picks);
          }
        }
      } catch (e) { /* picks fetch is best-effort */ }
      ...
      if (changed) {
        ...
        saveLocks(newLocked, newLockedFF); // Persist to Redis
