...
export default function useLiveScores() {
  ...
  const [cachedOdds, setCachedOdds] = useState(() => {
    ...
    try { return JSON.parse(localStorage.getItem("bf_odds_cache") || "{}"); }
    catch { return {}; }
  });
  ...
  // Survives page reloads, browser restarts, etc.
  const loadSavedOdds = useCallback(() => {
    if (typeof window === "undefined") return {};
    try { return JSON.parse(localStorage.getItem("bf_odds_cache") || "{}"); }
    catch { return {}; }
  }, []);
  ...
  const saveOdds = useCallback((cache) => {
    if (typeof window === "undefined") return;
    try { localStorage.setItem("bf_odds_cache", JSON.stringify(cache)); }
    catch { /* storage full or unavailable */ }
  }, []);

  const oddsCache = useRef(loadSavedOdds());
